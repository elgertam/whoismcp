#!/usr/bin/env python3
"""
Core MCP server logic shared between different transport implementations.
This module contains transport-agnostic MCP protocol handling.
"""
...
class MCPServerCore:
    """
    Core MCP server implementation with transport-agnostic protocol handling.

    This class handles all MCP protocol logic (tools, resources, request processing)
    independent of the transport layer (stdio, SSE, etc.).
    """

    def __init__(self, config: Config | None = None, rate_limit_client_id: str = "mcp_client") -> None:
        """
        Initialize MCP server core.

        Args:
            config: Configuration object (uses default if None)
            rate_limit_client_id: Client identifier for rate limiting
        """
        self.config = config or Config.from_env()
        self.rate_limit_client_id = rate_limit_client_id
    ...
    async def _handle_whois_lookup(self, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        # Check rate limiting
        if not await self.rate_limiter.acquire(self.rate_limit_client_id):
            return {
                "isError": True,
                "content": [
                    {
